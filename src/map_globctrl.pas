...
Uses
  ...
  main, dm, map_datastreams, map_geometry, map_wkt, Dialogs;
...
Type
  TCoordinates = Packed Object(map_geometry.T3DCoordinates)
...
Begin
  ...
  Location.Lat := (SMinLat+SMaxLat)/2;
  Location.Lon := (SMinLon+SMaxLon)/2;
  Center := Location;
  ReCenter;
end;
...
Begin
  ...
  Location.Lat := frmdm.CDS.FieldByName('latitude').AsFloat;
  Location.Lon := frmdm.CDS.FieldByName('longitude').AsFloat;
  Center := Location;
  ReCenter;
end;
...
Begin
 Marker.Lat := frmdm.CDS.FieldByName('latitude').AsFloat;
 Marker.Lon := frmdm.CDS.FieldByName('longitude').AsFloat;
 Refresh;
end;
  ...
  With FBufferBitmap.Canvas Do
    Begin
     ...
     { Draw stations }
      ...
      try
       cur_id:=frmdm.CDS.FieldByName('id').AsInteger;

       SetLength(X_arr,  StCount);
       SetLength(Y_arr,  StCount);
       SetLength(ID_arr, StCount);
